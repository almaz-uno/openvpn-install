...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# Configuration
			echo 'interface: 10.8.12.1
access-control: 10.8.12.1/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: 10.8.12.1|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: 10.8.12.1/24 allow|' /etc/unbound/unbound.conf
...
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS 10.8.12.1"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
...
verb 3" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s 10.8.12.0/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s 10.8.12.0/24 -o $NIC -j MASQUERADE
